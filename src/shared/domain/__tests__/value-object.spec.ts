...
class complexValueObject extends ValueObject {
    ...
    describe('ValueObject Unit Tests', () => {
        test('sould be equals', () => {
            const valueObject1 = new StringValueObject('test');
            const valueObject2 = new StringValueObject('test');
            expect(valueObject1.equals(valueObject2)).toBe(true);
            
            
            const complexValueObject1 = new StringValueObject("test" + 1);
            const complexValueObject2 = new StringValueObject("test" + 1);
            expect(complexValueObject1.equals(complexValueObject2)).toBe(true);
        });

            test('sould not be equals', () => {
        const valueObject1 = new StringValueObject('test');
